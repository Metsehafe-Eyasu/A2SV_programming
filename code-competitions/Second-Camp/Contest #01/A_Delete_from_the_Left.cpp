...
public:
    ...
    int startsWith(string prefix, int size) {
        Node* runner = root;
        int ans = 0;
        int n = prefix.length();
        FORR(i, n) {
            int index = prefix[i] - 'a';
            if (runner->children[index] == NULL || runner->children[index]->val < size)
                break;
            ans++;
...
void solve() {
    ...
    trie->insert(a);
    trie->insert(b);
    int common = trie->startsWith(a, 2);
    int ans = a.size() + b.size() - 2*common;
    cout << ans << "\n";
}
